...
def decrypt_block_at_offset(input_path: str, offset: int, output_dir: str = None) -> SCBlock:
    """直接跳转到特定偏移位置解密并提取数据"""
    import os
    # 读取文件
    with open(input_path, 'rb') as f:
    ...
    if output_dir:
        ...
        if block.Key == 0x2985FE5D:  # KParty的键值
            output_path = f"{output_dir}/KPartyData.bin"
            print(f"调试：找到KParty块，保存到 {output_path}")
        elif block.Key == 0x0D66012C:  # KBox的键值
            output_path = f"{output_dir}/KBoxData.bin"
...
def process_main_file(main_file_path: str, output_dir: str = ""):
    """处理main文件，解密Party和Box块并生成JSON文件"""
    import os
    import sys
    ...
    try:
        ...
        print("警告：无法导入file_manager模块，将使用默认文件操作")
        safe_load_file = None
        safe_save_file = None
    ...
    print(f"处理main文件: {main_file_path}")
    ...
    kparty_block = decrypt_block_at_offset(main_file_path, kparty_offset)  # 不保存到文件
    
    # print(f"\n尝试直接解密KBox块（偏移量: {kbox_offset}）...")
    kbox_block = decrypt_block_at_offset(main_file_path, kbox_offset)  # 不保存到文件
    
    # 检查是否成功解密了两个块
    if kparty_block is None:
        print(f"错误：未能成功解密KParty块")
        return False
    
    if kbox_block is None:
        print(f"错误：未能成功解密KBox块")
        return False
    ...
    try:
        ...
        # 直接传递数据给analyze_pk8生成JSON文件
        print("\n=== 使用analyze_pk8生成JSON文件 ===")
        # 使用file_manager中的函数获取正确的config目录路径
        if safe_save_file:
            from file_manager import get_config_dir
            config_dir = get_config_dir()
            output_file = os.path.join(config_dir, "pokemon_main_info.json")
        else:
            output_file = os.path.join("config", "pokemon_main_info.json")
            
        json_file = analyze_pk8.generate_pokemon_main_info_json_from_data(
            kparty_block.data, 
            kbox_block.data, 
            encrypted=True, 
        ...
        print(f"\n=== 处理完成 ===")
        print(f"已成功生成JSON文件: config/pokemon_main_info.json")
        return True
        
    except ImportError as e:
        print(f"错误：无法导入analyze_pk8模块: {e}")
        return False
